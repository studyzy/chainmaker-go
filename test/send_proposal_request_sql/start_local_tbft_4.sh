...
if [ -z ${pid} ];then
    nohup ./chainmaker start -c ../config-sql/wx-org1/chainmaker.yml local-tbft > panic.log &
...
if [ -z ${pid2} ];then
    nohup ./chainmaker start -c ../config-sql/wx-org2/chainmaker.yml local-tbft > panic.log &
...
if [ -z ${pid3} ];then
    nohup ./chainmaker start -c ../config-sql/wx-org3/chainmaker.yml local-tbft > panic.log &
...
if [ -z ${pid4} ];then
    nohup ./chainmaker start -c ../config-sql/wx-org4/chainmaker.yml local-tbft > panic.log &
