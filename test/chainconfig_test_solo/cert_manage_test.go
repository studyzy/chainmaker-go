...
func TestCertAdd(t *testing.T) {
	...
	resp, err := native.UpdateSysRequest(sk, member, &native.InvokeContractMsg{TxId: txId, TxType: commonPb.TxType_INVOKE_CONTRACT,
		ChainId: CHAIN1, ContractName: commonPb.SystemContract_CERT_MANAGE.String(), MethodName: commonPb.CertManageFunction_CERT_ADD.String(), Pairs: pairs})
...
func TestCertDelete(t *testing.T) {
	...
	sk, member := native.GetUserSK(1)
	resp, err := native.UpdateSysRequest(sk, member, &native.InvokeContractMsg{TxId: txId, TxType: commonPb.TxType_INVOKE_CONTRACT,
		ChainId: CHAIN1, ContractName: commonPb.SystemContract_CERT_MANAGE.String(), MethodName: commonPb.CertManageFunction_CERTS_DELETE.String(), Pairs: pairs})
	processResults(resp, err)
}
...
func TestCertQuery(t *testing.T) {
	...
	resp, err := native.QueryRequest(sk, member, &client, &native.InvokeContractMsg{TxType: commonPb.TxType_QUERY_CONTRACT,
		ChainId: CHAIN1, ContractName: commonPb.SystemContract_CERT_MANAGE.String(), MethodName: commonPb.CertManageFunction_CERTS_QUERY.String(), Pairs: pairs})
	processResults(resp, err)
}
