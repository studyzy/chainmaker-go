...
func constructAC(t *testing.T, info orgInfo) ac {
	...
	chainConf.Crypto.Hash = info.hashType
	acInst, err := newAccessControlWithChainConfigPb(chainConf, info.orgId, nil, logger)
	require.Nil(t, err)
	...
	require.Nil(t, err)
	consensusNode, err := InitCertSigningMember(info.hashType, info.orgId, localPrivKeyFile, "", localCertFile)
	require.Nil(t, err)

	err = ioutil.WriteFile(localPrivKeyFile, []byte(info.commonNode.sk), os.ModePerm)
	require.Nil(t, err)
	err = ioutil.WriteFile(localCertFile, []byte(info.commonNode.certificate), os.ModePerm)
	require.Nil(t, err)
	commonNode, err := InitCertSigningMember(info.hashType, info.orgId, localPrivKeyFile, "", localCertFile)
	require.Nil(t, err)

	err = ioutil.WriteFile(localPrivKeyFile, []byte(info.admin.sk), os.ModePerm)
	require.Nil(t, err)
	err = ioutil.WriteFile(localCertFile, []byte(info.admin.certificate), os.ModePerm)
	require.Nil(t, err)
	admin, err := InitCertSigningMember(info.hashType, info.orgId, localPrivKeyFile, "", localCertFile)
	require.Nil(t, err)

	err = ioutil.WriteFile(localPrivKeyFile, []byte(info.client.sk), os.ModePerm)
	require.Nil(t, err)
	err = ioutil.WriteFile(localCertFile, []byte(info.client.certificate), os.ModePerm)
	require.Nil(t, err)
	client, err := InitCertSigningMember(info.hashType, info.orgId, localPrivKeyFile, "", localCertFile)
	require.Nil(t, err)
	return ac{
		acInst:        acInst,
