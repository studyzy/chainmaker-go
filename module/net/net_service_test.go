/*
Copyright (C) BABEC. All rights reserved.

SPDX-License-Identifier: Apache-2.0
*/

package net

import (
	"fmt"
	"io/ioutil"
	"os"
	"path/filepath"
	"testing"
	"time"

	netPb "chainmaker.org/chainmaker/pb-go/net"
	"chainmaker.org/chainmaker/protocol"
	"github.com/stretchr/testify/require"
)

const (
	keyBegin = "-----BEGIN EC PRIVATE KEY-----"
	keyEnd   = "-----END EC PRIVATE KEY-----"
	crtBegin = "-----BEGIN CERTIFICATE-----"
	crtEnd   = "-----END CERTIFICATE-----"
	seed1    = "QmeyNRs2DwWjcHTpcVHoUSaDAAif4VQZ2wQDQAUNDP33gH"
	seed2    = "QmcQHCuAXaFkbcsPUj7e37hXXfZ9DdN7bozseo5oX4qiC4"

	key6666  = keyBegin + "\nMHcCAQEEIF4Sy4KANZHi8uU4YkmymbcbF3HHJnGgSjV/0iNOSdy3oAoGCCqGSM49\nAwEHoUQDQgAEKwemRhrzv5GSSmsy4EREhnQJ4jocauyWnD1dXUx9X8c4VwhG5hWQ\n7oc+cMyz6rXPKTrUxKD50V+OB0FVkpY7vA==\n" + keyEnd + "\n"
	cert6666 = crtBegin + "\\n" + crtEnd + "\n"
	ca6666   = crtBegin + "\\n" + crtEnd + "\n"
	key7777  = keyBegin + "\nMHcCAQEEIIimV5TA1i8QWlp5nD5r5KmpueJV1hplp5y7Of4CYquzoAoGCCqGSM49\nAwEHoUQDQgAESZXYY4gziokaliXX5JkwT+idTCCwesjuJtTupABuhIqu7o2jt1V0\nNNWVvpShIM+878BaSb2v2TllwVoOYmfzPg==\n" + keyEnd + "\n"
	cert7777 = crtBegin + "\\n" + crtEnd + "\n"
	ca7777   = crtBegin + "\\n" + crtEnd + "\n"
)

func TestNetService(t *testing.T) {
	var td = filepath.Join(os.TempDir(), "temp")
	err := os.MkdirAll(td, os.ModePerm)
	require.Nil(t, err)
	defer func() {
		_ = os.RemoveAll(td)
		_ = os.RemoveAll(filepath.Join("./default.log"))
		now := time.Now()
		_ = os.RemoveAll(filepath.Join("./default.log." + now.Format("2006010215")))
		now = now.Add(-5 * time.Hour)
		_ = os.RemoveAll(filepath.Join("./default.log." + now.Format("2006010215")))
	}()

	require.Nil(t, ioutil.WriteFile(filepath.Join(td, "6666.key"), []byte(key6666), 0777))   //nolint: gosec
	require.Nil(t, ioutil.WriteFile(filepath.Join(td, "6666.crt"), []byte(cert6666), 0777))  //nolint: gosec
	require.Nil(t, ioutil.WriteFile(filepath.Join(td, "6666.ca.crt"), []byte(ca6666), 0777)) //nolint: gosec
	require.Nil(t, ioutil.WriteFile(filepath.Join(td, "7777.key"), []byte(key7777), 0777))   //nolint: gosec
	require.Nil(t, ioutil.WriteFile(filepath.Join(td, "7777.crt"), []byte(cert7777), 0777))  //nolint: gosec
	require.Nil(t, ioutil.WriteFile(filepath.Join(td, "7777.ca.crt"), []byte(ca7777), 0777)) //nolint: gosec
	caBytes6666, err := ioutil.ReadFile(filepath.Join(td, "6666.ca.crt"))
	require.Nil(t, err)
	caBytes7777, err := ioutil.ReadFile(filepath.Join(td, "7777.ca.crt"))
	require.Nil(t, err)

	// start node A
	a, nsa, err := start(
		"A", td, "6666.key", "6666.crt", "/ip4/127.0.0.1/tcp/6666", seed1, "", caBytes6666, caBytes7777)
	require.Nil(t, err)
	//start node B
	b, nsb, err := start(
		"B",
		td,
		"7777.key",
		"7777.crt",
		"/ip4/127.0.0.1/tcp/7777",
		seed2,
		"/ip4/127.0.0.1/tcp/6666/p2p/"+seed2,
		caBytes6666,
		caBytes7777,
	)
	require.Nil(t, err)

	// test A send msg to B
	data := []byte("hello")
	passChan := make(chan bool)
	timer := time.NewTimer(time.Minute)
	testNetServiceSendMsg(t, nsa, nsb, passChan, data, timer)

	// test broadcast
	subHandlerB := func(_ string, msg []byte, _ netPb.NetMsg_MsgType) error {
		fmt.Println("[B][chain1] recv a sub msg chain1：", string(msg))
		passChan <- true
		return nil
	}
	require.Nil(t, nsb.Subscribe(netPb.NetMsg_TX, subHandlerB))
	fmt.Println("[B]B subscribe topic of chain1")
	require.Nil(t, nsa.BroadcastMsg(data, netPb.NetMsg_TX))
	fmt.Println("[A]A broadcast a msg to chain1:", string(data))

	timer = time.NewTimer(time.Minute)
	select {
	case <-timer.C:
		fmt.Println("==== test broadcast timeout ====")
		t.Fatal("test broadcast failed")
	case <-passChan:
		fmt.Println("==== test broadcast pass ====")
	}

	// test cancel broadcast
	require.Nil(t, nsb.CancelSubscribe(netPb.NetMsg_TX))
	fmt.Println("[B]B cancel subscribe topic of chain1")
	require.Nil(t, nsa.BroadcastMsg(data, netPb.NetMsg_TX))
	fmt.Println("[A]A broadcast a msg to chain1:", string(data))

	timer = time.NewTimer(10 * time.Second)
	select {
	case <-timer.C:
		fmt.Println("==== test cancel broadcast pass ====")
	case <-passChan:
		fmt.Println("==== test cancel broadcast failed ====")
		t.Fatal("test cancel broadcast failed")
	}

	// test consensus broadcast
	require.Nil(t, nsb.ConsensusSubscribe(netPb.NetMsg_TX, subHandlerB))
	fmt.Println("[B]B subscribe consensus topic of chain1")
	require.Nil(t, nsa.ConsensusBroadcastMsg(data, netPb.NetMsg_TX))
	fmt.Println("[A]A broadcast a msg to chain1 consensus nodes:", string(data))

	timer = time.NewTimer(time.Minute)
	select {
	case <-timer.C:
		fmt.Println("==== test consensus broadcast timeout ====")
		t.Fatal("test consensus broadcast failed")
	case <-passChan:
		fmt.Println("==== test consensus broadcast pass ====")
	}

	// test cancel consensus broadcast
	require.Nil(t, nsb.CancelConsensusSubscribe(netPb.NetMsg_TX))
	fmt.Println("[B]B cancel subscribe consensus topic of chain1")
	require.Nil(t, nsa.ConsensusBroadcastMsg(data, netPb.NetMsg_TX))
	fmt.Println("[A]A broadcast a msg to chain1 consensus nodes:", string(data))

	timer = time.NewTimer(10 * time.Second)
	select {
	case <-timer.C:
		fmt.Println("==== test cancel consensus broadcast pass ====")
	case <-passChan:
		fmt.Println("==== test cancel consensus broadcast failed ====")
		t.Fatal("test cancel consensus broadcast failed")
	}

	require.Nil(t, nsa.Stop())
	require.Nil(t, a.Stop())
	require.Nil(t, nsb.Stop())
	require.Nil(t, b.Stop())
}

func start(nodeName, tempDir, keyPath, crtPath, netAddr, netServiceAddrSeed, addSeed string, trustRootList ...[]byte) (Net, protocol.NetService, error) {
	var nf NetFactory

	n, err := nf.NewNet(
		protocol.Libp2p,
		//WithListenAddr("/ip4/127.0.0.1/tcp/6666"),
		WithListenAddr(netAddr),

		WithCrypto(filepath.Join(tempDir, keyPath), filepath.Join(tempDir, crtPath)),
	)
	if err != nil {
		return nil, nil, err
	}

	if addSeed != "" {
		if err = n.AddSeed(addSeed); err != nil {
			return nil, nil, err
		}
	}

	//n.AddSeed("/ip4/127.0.0.1/tcp/7777/p2p/" + seed1)
	for _, caBytes := range trustRootList {
		if err = n.AddTrustRoot(chainId1, caBytes); err != nil {
			return nil, nil, err
		}
	}

	if err = n.Start(); err != nil {
		return nil, nil, err
	}

	fmt.Printf("node %s is running...\n", nodeName)

	var nsf NetServiceFactory
	ns, err := nsf.NewNetService(
		n,
		chainId1,
		nil,
		nil,
		//WithConsensusNodeUid(seed1),
		WithConsensusNodeUid(netServiceAddrSeed),
	)
	if err != nil {
		return nil, nil, err
	}
	if err = ns.Start(); err != nil {
		return nil, nil, err
	}

	fmt.Printf("net service %s is running...\n", nodeName)
	return n, ns, nil

}

func testNetServiceSendMsg(t *testing.T, src, det protocol.NetService, passChan chan bool, data []byte, timer *time.Timer) {
	toNodeB := seed1
	recHandlerB := func(id string, msg []byte, _ netPb.NetMsg_MsgType) error {
		fmt.Println("[B][chain1] recv a msg from peer[", id, "], msg：", string(msg))
		passChan <- true
		return nil
	}
	require.Nil(t, det.ReceiveMsg(netPb.NetMsg_TX, recHandlerB))
	fmt.Println("[B]B register receive msg handler for chain1")
	go func() {
		fmt.Println("[A]A send msg to B in chain1")
		for {
			if err := src.SendMsg(data, netPb.NetMsg_TX, toNodeB); err != nil {
				fmt.Println(err)
				time.Sleep(time.Second)
				continue
			}
			break
		}
	}()
	select {
	case <-timer.C:
		fmt.Println("==== test A send msg to B timeout ====")
		t.Fatal("test A send msg to B timeout")
	case <-passChan:
		fmt.Println("==== test A send msg to B pass ====")
	}
}
