...
func TestNet(t *testing.T) {
	...
	testNetSendMsg(t, a, b, passChan, data, toNodeB, timer)
	...
	require.Nil(t, b.SubscribeWithChainId(chainId1, testTopic, subHandlerB))
	fmt.Println("[B]B subscribe topic of chain1")
	sendMsg := NewNetMsg(data, netPb.NetMsg_TX, toNodeB)
	require.Nil(t, a.BroadcastWithChainId(chainId1, testTopic, sendMsg))
	...
	require.Nil(t, b.CancelSubscribeWithChainId(chainId1, testTopic))
...
func testNetSendMsg(t *testing.T, src, det Net, passChan chan bool, data []byte, toTargetNode string, timer *time.Timer) {
	recHandlerB := func(id string, msg *netPb.NetMsg) error {
		fmt.Println("[B][chain1] recv a msg from peer[", id, "], msg：", string(msg.GetPayload()))
		passChan <- true
		return nil
	}
	err := det.DirectMsgHandle(chainId1, msgFlag, recHandlerB)
	require.Nil(t, err)
	fmt.Println("[B]B register receive msg handler for chain1")
	go func() {
		fmt.Println("[A]A send msg to B in chain1")
		for {
			sendMsg := NewNetMsg(data, netPb.NetMsg_TX, toTargetNode)
			if err = src.SendMsg(chainId1, toTargetNode, msgFlag, sendMsg); err != nil {
				fmt.Println(err)
